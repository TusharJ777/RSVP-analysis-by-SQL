...
-- Number of movies released based on the YEAR.
 SELECT year, COUNT(id) as number_of_movies
FROM movie
GROUP BY year
ORDER BY year;

-- Number of movies based on the MONTH.
...
where g.genre in (select genre from top_genres) and avg_rating>8
...
( select d.name_id, name, d.movie_id, m.date_published, lead(date_published, 1) 
over(partition by d.name_id order by date_published, d.movie_id) as next_movie_date
...
sum(duration) as total_duration, row_number() over(order by count(d.movie_id) desc) as director_row_rank
from names as n 
